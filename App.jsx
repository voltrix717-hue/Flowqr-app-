...
const OWNER_PASSCODE = "202622"; // acceso al panel del propietario
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (res && res.value) setMiEmpresa(JSON.parse(res.value));
      } catch (e) {
  ...
  async function registrarEmpresa(nombre, codigo) {
    ...
    await window.storage.set(MI_EMPRESA_KEY, JSON.stringify(empresa), false);
  ...
  if (!miEmpresa) {
    return (
      <div className="fq-root">
        <Style />
        <RegistroEmpresaScreen onRegistrar={registrarEmpresa} onOwnerAccess={() => setOwnerMode(true)} />
      </div>
    );
  }
  ...
  return (
    <div className="fq-root">
      <Style />
      <TopBar role={role} setRole={setRole} saveError={saveError} empresa={miEmpresa} onSalir={salirDeEmpresa} />
...
/* ------------------------- registro de empresa -------------------------- */
function RegistroEmpresaScreen({ onRegistrar, onOwnerAccess }) {
  ...
  function intentarOwner() {
    if (passcode === OWNER_PASSCODE) {
      onOwnerAccess();
    } else {
      setOwnerError("Código de propietario incorrecto.");
    }
  }
...
/* ------------------------------ Top bar -------------------------------- */
function TopBar({ role, setRole, saveError, empresa, onSalir }) {
  return (
...
/* ============================== ADMIN ================================== */
function AdminPanel({ lotes, updateLote, addLote, maquinas, addMaquina, updateMaquina, config, updateConfig }) {
